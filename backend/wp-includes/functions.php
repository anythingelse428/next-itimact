...
add_action('graphql_init', function () {
  $featuredImageUrl = [
    'type' => 'String',
    'description' => __('Custom field for post mutations', 'wp-graphql'),
    'resolve' => function ($post) {
      $featuredImageUrl = get_post_meta($post->ID, 'featuredImageUrl', true);
      return !empty($featuredImageUrl) ? $featuredImageUrl : '';
    },
  ];

  register_graphql_field('Post', 'featuredImageUrl', $featuredImageUrl);
  register_graphql_field('CreatePostInput', 'featuredImageUrl', $featuredImageUrl);
  register_graphql_field('UpdatePostInput', 'featuredImageUrl', $featuredImageUrl);
});

add_action('graphql_register_types', function () {

  register_graphql_mutation('featuredImageUrlMutation', [
    'inputFields' => [
      'featuredImageUrl' => [
        'type' => ['non_null' => 'String'],
      ],
...
add_action('graphql_post_object_mutation_update_additional_data', 'graphql_register_post_mutation', 10, 5);

function graphql_register_post_mutation($post_id, $input, $mutation_name, $context, $info)
{
  if (isset($input['featuredImageUrl'])) {
    // Consider other sanitization if necessary and validation such as which
    // user role/capability should be able to insert this value, etc.
    update_post_meta($post_id, 'featuredImageUrl', $input['featuredImageUrl']);
  }
}
...
add_action('graphql_init', function () {
  $description = [
    'type' => 'String',
    'description' => __('Text in feed. Field for post mutations', 'wp-graphql'),
    'resolve' => function ($post) {
  ...
  register_graphql_field('Post', 'description', $description);
  register_graphql_field('CreatePostInput', 'description', $description);
  register_graphql_field('UpdatePostInput', 'description', $description);
});

add_action('graphql_register_types', function () {

  register_graphql_mutation('descriptionMutation', [
    'inputFields' => [
      'description' => [
        'type' => ['non_null' => 'String'],
      ],
...
add_action('graphql_post_object_mutation_update_additional_data', 'graphql_register_post_mutation_description', 10, 5);

function graphql_register_post_mutation_description($post_id, $input, $mutation_name, $context, $info)
{
  if (isset($input['description'])) {
    // Consider other sanitization if necessary and validation such as which
    // user role/capability should be able to insert this value, etc.
    update_post_meta($post_id, 'description', $input['description']);
  }
}